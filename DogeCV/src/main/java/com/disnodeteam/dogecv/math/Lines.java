...
public class Lines {
	...
	/**
	 * Returns the sign of the z-component (the only non-zero component) of the cross product of two vectors represented by a line and a point, such that the relative position of a point to a line can be determined.
	 * If the two vectors are named u and v, the sign will be:
	 *  - Positive if v is counterclockwise from u (on the standard x-y plane)
	 *  - Negative if v is clockwise from u
	 *  - Zero if v is parallel to u (i.e. points in the same or opposite direction as u)
	 *
	 ...
	 * @return The sign of the z-component of the cross product of the vectors
	 */
	...
	public static Line getPerpendicular(Line line, double sign) {
		double angle = Lines.getAngularDistance(line, new Line(new Point(0, 0), new Point(100, 0)));
		angle -= Math.signum(sign) * 90;
		...
		return new Line(line.center(), new Point(x, y));
	}

	/**
	 * @return The distance between the given line1 and the line passing through the center of line2 that is parallel to line1
	 */
	public static double getPerpendicularDistance(Line line1, Line line2) {
		Line perp = Lines.getPerpendicular(line1, Lines.crossSign(line1, line2.center()));
	...
	public static double getPerpendicularDistance(Line line1, Point point) {
		Line perp = Lines.getPerpendicular(line1, Lines.crossSign(line1, point));
		Line joint = new Line(line1.center(), point);
		return Math.cos(Lines.getAngularDistance(perp, joint) * Math.PI / 180) * joint.length();
	}

	public static Line getPerpendicularConnector(Line left, Line right, Size size) {
	...
	public static List<List<Line>> groupLines(ArrayList<Line> lines, double separation) {
		...
		for (List<Line> b : MathFTC.combinations(lines, 2)) {
			...
			if (Lines.getPerpendicularDistance(b.get(0), b.get(1)) < separation) {
		...
		for (Joint joint : joints) {
			...
			for (int i = 0; i < mergeCollections.size(); i++) {
				if (Lines.getPerpendicularDistance(joint.line1, mergeCollections.get(i).center) < separation) {
					close = i;
					break;
				}
			}
