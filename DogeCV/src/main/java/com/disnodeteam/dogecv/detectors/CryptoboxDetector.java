...
public class CryptoboxDetector extends OpenCVPipeline {
	...
	private final List<RailTimeline> output = new ArrayList<>();
	...
	public Mat processFrame(Mat rgba, Mat gray) {
		...
		if (linesVertical.size() == 0) {
			...
			// Create a list of lines representing the columns (as opposed to groups of lines)
	...
	private static class RailTimeline {
		...
		public int framesSinceLastSeen() {
			for (int n = 0; n < positions.size(); n++) {
				if (!Double.isNaN(positions.get(n))) return n;
			}
			return positions.size();
		}

		public double lastKnownPosition() {
			return positions.get(framesSinceLastSeen());
		}
