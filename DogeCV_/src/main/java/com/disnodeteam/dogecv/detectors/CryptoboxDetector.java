...
public class CryptoboxDetector extends OpenCVPipeline {
	...
	public Mat processFrame(Mat rgba, Mat gray) {
		...
		for (int i = 0; i < groupings.size() - 1; i++) {
			...
			while (y < mask.height() && y < max && count < 10) {
				if (mask.get(y, (int) center.x)[0] > 0) {
					count++;
					// Imgproc.circle(original, new Point(2*center.x, 2*y), 10, new
		...
		for (int i = 0; i < groupings.size(); i++) {
			...
			int count = 0;
			Log.d("DogeCV", String.format(Locale.ENGLISH, "Size: (%d, %d), Center: (%.2f, %.2f), Max: %.2f, MinMax X: (%d, %d)", mask.width(), mask.height(), center.x, center.y, max, minX, maxX));
			while (y < mask.height() && y < max && count < 10) {
				if (mask.get(y, (int) center.x)[0] > 0 || mask.get(y, minX)[0] > 0 || mask.get(y, maxX)[0] > 0) {
					count++;
					// Imgproc.circle(rgba, new Point(2*center.x, 2*y), 10, new Scalar(255,255,255),
					// 6);
				} else {
					// Imgproc.circle(rgba, new Point(2*center.x, 2*y), 10, new Scalar(30,30,200),
					// 6);
				}
				y += 4;
			}
		...
		for (int i = 0; i < groupings.size(); i++) {
			// Imgproc.circle(original, Lines.getMeanPoint(groupings.get(i)), 50, new
			// Scalar(40,200,70), 4);
			for (Line line : groupings.get(i)) {
				...
				// Imgproc.putText(rgba, Integer.toString(i), line.center(),
				// Core.FONT_HERSHEY_PLAIN, 7, new Scalar(10,240,230),3);
			}
		}
	...
	public Point drawSlot(int slot, List<Rect> boxes) {
		...
		int drawY = leftColumn.height + leftColumn.y; // Calculate Y Coord. We wont use this in our
														// bot's opetation, buts its nice for
														// drawing
